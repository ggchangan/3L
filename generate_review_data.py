...
def get_buy_sell_signals(holdings, buy_signals, date_str=None):
    ...
    try:
        ...
        # 找date_str对应的K线索引（如果没有date_str就用全部数据）
...
def is_trading_day(date_str):
    ...
    import json as _json
    cache_file = os.path.join(WWW_DIR, 'private', 'trading_days_cache.json')
    # 尝试从缓存加载
    trading_days = None
    ...
    if trading_days is None:
        try:
            ...
            with open(cache_file, 'w') as _f:
                _json.dump({'cached_at': datetime.now().strftime('%Y-%m-%d'), 'days': trading_days}, _f)
...
def generate_daily_review(date_str=None):
    ...
    buy_signals, all_stocks_60d = scan_buy_signals_if_needed(
        buy_signals, all_stocks_60d if "all_stocks_60d" in dir() else None,
        date_str, WWW_DIR, ALL_STOCKS_PATH,
        mainline_data, market_cycle, get_watchlist,
    )

    # 从最新 buy_signals 生成持仓信号（必须在扫描之后）
    timing_signals, stock_cache, bs_by_code = get_buy_sell_signals(holdings, buy_signals, date_str)

    # 确保 all_stocks_60d 已加载（用于 review_analysis 模块）
    if 'all_stocks_60d' not in dir() or not all_stocks_60d:
        try:
            _as60_path = ALL_STOCKS_PATH
            if os.path.isfile(_as60_path):
                with open(_as60_path) as _f:
    ...
    # 使用 review_analysis 模块生成持仓复盘和买点信号复盘
    from scripts.review_analysis import generate_holdings_review, generate_buy_signals_review
...
def generate_daily_achievements_pdf(date_str):
    ...
    if os.path.isfile(review_file):
        try:
            ...
            logger.warning("[3L复盘] 无法读取main_lines")
