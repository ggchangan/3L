...
def trend_of(code, d):
    """返回 '上升趋势' / '下降趋势' / '区间震荡'
    实践标准：MA10向上=上升，MA10向下=下降，其余=震荡
    来源：3L体系理论的实践简化
    可替换/优化：如需更精细判断可改为均线系统+波段结构"""
    kl = KL(code)
    ...
    diff = (ma10_now - ma10_prev) / ma10_prev
...
print("═══ v3.2 第1周 (20260407~20260410) ═══")
print("新增：个股走势结构过滤（MA10斜率，实践标准，可替换）")
sim = Sim()
ph,lim,mult = jm("20260407")
print(f"大盘: {ph} (量比{mult}x)")
...
# 建仓
cs = scan("20260407",[])
print(f"中继买点候选（已过滤下降趋势）: {len(cs)}只")
for c in cs[:5]:
    print(f"  {nm(c['code'])}({c['code']}) {c['sec']} 趋势:{c['trend']} 量比:{c['vr']}")
...
for c in cs:
    ...
    if pct>0 and tp+pct*100 <= 80:
        sim.buy("20260407",code,pct,det,c)
        exs.add(sec)
        print(f"  ✅ {nm(code)}({code}) {sec} {pct*100:.1f}% | {c.get('trend','')}")
...
for d in ["20260408","20260409","20260410"]:
    sim.check(d)
    tv = sim.tv(d)
    print(f"  {d[-5:]}: {len(sim.pf)}只 tv={tv:.0f} pnl={tv-1000000:+.0f}")

end = sim.tv("20260410")
print(f"\n📊 第1周: +{end-1000000:+,.0f} ({(end-1000000)/1000000*100:+.2f}%) | 仓位{sim.tpct('20260410'):.1f}%")
print(f"\n📋 交易明细:")
for t in sim.trades:
    if t["direction"]=="买入":
        print(f"  {t['date'][-5:]} 买入 {t['name']:12} {t['sector']:8} {t['qty']:>5}股 {t['price']:>7.2f} | {t['reason']} 个股{t['pos_pct']:>5}% 总{t['total_pct']:>5}% 止损{t.get('stop_loss',0):.2f}")
    elif t["direction"]=="卖半":
...
pickle.dump({"cash":sim.cash,"pf":sim.pf,"trades":sim.trades,"end":end}, open(os.path.join(OUT,"w1_v32.pkl"),"wb"))
print(f"\n✅ 保存")
