...
class Sim:
    def __init__(self):
        self.cash = 1000000
        self.pf = {}  # {code: {shares,price,cost,entry_date,pct,sl(关键支撑价),hwm,consec_gains,last5_chg}}
        self.trades = []
        self.ph = ""
    ...
    def tv(self,d):
        return self.cash + sum(self.px(c,p["entry_date"] if d=="20260407" else d)*p["shares"] for c,p in self.pf.items())
    ...
    def buy(self,d,c,pct,det,entry_data):
        kl = KL(c)
        ...
        self.cash -= cost
        # 止损设在买入当天的回踩支撑下方一点
        sp = entry_data["support"]
        sp_val = entry_data["ma10"] if sp=="MA10" else entry_data["ma20"]
        sl = sp_val * 0.97  # 支撑下方3%
        self.pf[c] = {"shares":sh,"price":p,"cost":cost,"entry_date":d,"pct":pct,
                      "sl":round(sl,2),"hwm":p,"consec_gains":0,"last5_chg":[0]*5}
        tp = self.tpct(d)
        self.trades.append({"date":d,"time":"15:00","direction":"买入","code":c,
            ...
            "reason":"中继买点","stop_loss":round(sl,2)})
        return True
    
    def sell(self,d,c,reason):
    ...
    def check(self,d):
        ...
        for c in list(self.pf.keys()):
            ...
            if idx>0:
                ...
                pos["last5_chg"] = (pos["last5_chg"]+[chg])[-5:]
                if chg>0: pos["consec_gains"]+=1
                else: pos["consec_gains"]=0
            
            # 止损：跌破关键支撑
            if l>0 and l<=pos["sl"]:
                self.sell(d,c,f"跌破止损{pos['sl']:.2f}")
                continue
            ...
            # ① 加速：连续大阳+量放大
            if pos["consec_gains"]>=3 and v_ma5>0:
                last3 = pos["last5_chg"][-3:]
                # 每日涨幅逐步扩大
                if last3[0]<last3[1]<last3[2] and V[idx]>v_ma5:
                    self.sell(d,c,f"加速|连阳{pos['consec_gains']}天量放{round(V[idx]/v_ma5,1)}x")
                    continue
            
            # ② 放量滞涨：量放大+涨幅比之前小
            if v_ma5>0 and V[idx]>v_ma5*1.5:
                avg_prev = sum(pos["last5_chg"][:-1])/max(len(pos["last5_chg"][:-1]),1) if len(pos["last5_chg"])>1 else 0
                if avg_prev>0 and pos["last5_chg"][-1] < avg_prev*0.5:
                    self.sell(d,c,f"放量滞涨|量比{round(V[idx]/v_ma5,1)}x涨幅{pos['last5_chg'][-1]:.1f}%")
                    continue
            
            # ③ 动力减弱：价创新高+量递减
            if idx>=3 and p>=pos["hwm"]*0.995:  # 接近或创新高
                if V[idx-2]>V[idx-1]>V[idx]:
                    self.sell(d,c,f"动力减弱|价高量缩连3日")
                    continue
            
            # 右侧止盈：跌破MA20
            ma20 = sum(C[idx-19:idx+1])/20
            if p<ma20:
                self.sell(d,c,f"跌破MA20({ma20:.2f})")
                continue
    ...
    def prev_day(self,d):
        for sec,stocks in ALL.items():
            for c in stocks:
                ...
                if kl:
                    idx = DI(d,kl)
                    if idx>0: return kl[idx-1]["date"]
        return d

print("═══ v3 第1周 (20260407~20260410) ═══")
sim = Sim()
