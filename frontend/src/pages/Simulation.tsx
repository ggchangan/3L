...
interface Trade {
  date: string; dir: string; code: string; name: string
  sector: string; type: string; qty: number; price: number
}

const WEEKS = ['w1', 'w2', 'w3', 'w4']
const WEEK_LABELS: Record<string, string> = {
  w1: '第1周 (4/7~4/10)', w2: '第2周 (4/13~4/17)',
  w3: '第3周 (4/20~4/24)', w4: '第4周 (4/27~4/30)',
}
const WEEK_PNLS: Record<string, string> = { w1: '+7.68%', w2: '-0.88%', w3: '-2.10%', w4: '+3.34%' }
const WEEK_TRADES: Record<string, number> = { w1: 13, w2: 27, w3: 28, w4: 16 }
const WEEK_MAIN: Record<string, string> = {
  w1: '半导体', w2: '创新药→算力', w3: '商业航天→算力', w4: '半导体',
}
const WEEK_REPORT: Record<string, string> = {
  w1: '/pub/files/模拟交易v33_第1周报告.pdf',
  w2: '/pub/files/模拟交易v33_第2周报告.pdf',
  w3: '/pub/files/模拟交易v33_第3周报告.pdf',
  w4: '/pub/files/模拟交易v33_第4周报告.pdf',
}

const TRADE_DATA: Record<string, Trade[]> = {
  w1: [
  ...
  return (
    <div className="page-container">
      ...
      <div className="header">
        <h1>📊 3L 模拟交易 · v33</h1>
        <div className="sub">动量主线 · 最强逻辑 · 量价择时 · 细分赛道聚类</div>
      </div>
      ...
      <div className="container">
        ...
        <div className="summary-grid">
          <div className="summary-card">
            ...
            <div className="label">模拟周数</div>
          </div>
          <div className="summary-card">
            <div className="num" style={{ color: '#4ecdc4' }}>+2.01%</div>
            <div className="label">
              4周累计收益
              <a href="/pub/files/模拟交易v33_总体报告.pdf" target="_blank" style={{ color: '#4ecdc4', textDecoration: 'none', fontSize: 11, marginLeft: 4 }}>📄 总体</a>
            </div>
          </div>
          <div className="summary-card">
            <div className="num">84</div>
            <div className="label">总交易笔数</div>
          </div>
          <div className="summary-card">
            <div className="num">v33</div>
            <div className="label">引擎版本</div>
          </div>
        </div>
            ...
            return (
              <div key={wk} className="week-card">
                <div className="w-title">
                  ...
                  <span style={{ color: '#888', fontSize: 12, fontWeight: 'normal', marginLeft: 8 }}>
                    方向: {WEEK_MAIN[wk]} | {WEEK_TRADES[wk]}笔
                  </span>
                  <a href={WEEK_REPORT[wk]} target="_blank" style={{ color: '#4ecdc4', textDecoration: 'none', fontSize: 11, marginLeft: 8 }}>
                    📄 周报告
                  </a>
                </div>
                {trades.length > 0 && (
                  <table>
                    <thead>
                      <tr>
                        ...
                        <th>板块</th><th>买点类型</th><th>数量</th><th>价格</th>
                      </tr>
                    </thead>
                    <tbody>
                      {trades.map((t, i) => {
                        const dirClass = t.dir === '买入' ? 'buy' : t.dir === '卖出' ? 'sell' : 'half'
                        return (
                          <tr key={i}>
                            <td>{t.date}</td>
                            <td className={dirClass}>{t.dir}</td>
                            <td>{t.code}</td>
                            <td>{t.name}</td>
                            <td>{t.sector}</td>
                            <td>{t.type}</td>
      ...
      <div className="footer">3L 交易体系 · 模拟交易 v33 · Hermes Agent · 2026</div>
      <BottomNav />
    </div>
  )
}
