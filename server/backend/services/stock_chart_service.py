...
def _find_breakthrough_points(closes, highs, lows, volumes, structure=None, stage=None,
    ...
    关键点类型一览（静态层，基于量价数据实时计算）：
      - 前高：局部波峰（比前后各5根K线的最高都高）→ 橙色方块
      - 前低：局部波谷（比前后各5根K线的最低都低）→ 绿色方块
      - 突：真区间震荡突破（结构=区间震荡+前12根收盘价窄幅≤8%）→ 蓝色方块
      - 反：阴转阳反转形态 → 紫色方块（仅大盘/板块）
      - 放↑：放量上涨（≥1.5倍10日均量，涨幅>2%）→ 深橙
      - 放↓：放量下跌（≥1.5倍10日均量，跌幅>2%）→ 紫色
      - 缩：缩量（接近10日最低量50%）→ 灰色
      - ↯：放量滞涨（stage=放量滞涨+量>1.2倍）→ 橙色
