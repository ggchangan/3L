...
def _find_breakthrough_points(closes, highs, lows, volumes, structure=None, stage=None,
    ...
    关键点类型一览（静态层，基于量价数据实时计算，每个关键点代表供需格局转换）：
      - 前高：局部波峰，比前后各5根K线的最高都高 → 供应开始主导，需求衰竭信号
      - 前低：局部波谷，比前后各5根K线的最低都低 → 需求开始主导，供应衰竭信号
      - 突：真区间震荡突破（结构=区间震荡+前12根收盘价窄幅≤8%）→ 需求突破供应区
      - 反：阴转阳反转形态 → 短期供需逆转（仅大盘/板块）
      - 放↑：放量上涨（≥1.5倍10日均量，涨幅>2%）→ 需求主动进攻
      - 放↓：放量下跌（≥1.5倍10日均量，跌幅>2%）→ 供应主动砸盘
