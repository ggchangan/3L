...
describe('MainlineSection', () => {
  ...
  it('按10日板块强度候选分层，并把波峰波谷保留为板块阶段', () => {
    ...
    expect(screen.getByText('10日强度6–10候选')).toBeTruthy()
...
describe('板块环境语义', () => {
  it('不把波谷和趋势阶段直接称为机会', () => {
    expect(formatSectorEnvironment('主线回调')).toBe('10日强度前5候选 · 波谷')
    expect(formatSectorEnvironment('次线机会')).toBe('10日强度6–10候选 · 波谷')
    expect(formatSectorEnvironment('见顶风险', '主线')).toBe('10日强度前5候选 · 波峰风险')
