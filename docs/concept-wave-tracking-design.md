...
Excalidraw 线框图：https://excalidraw.com/#json=PMLdEqpnROCavWO0G--nf,azcOVOGYAwIhxjs29_9rfQ
...
在核心追踪列表上方加入一条**多概念归一化对比折线图**，以时间为横轴、归一化价格为纵轴，同时绘制所有核心概念板块的走势线。

**设计要点：**
- **归一化处理**：每个概念以 30 天前的价格为基准（=100），消除绝对值差异，只看相对涨跌节奏
- **颜色区分**：当前波谷的概念用绿色高亮，波峰用红色高亮，波中用黄色
- **标注**：自动标注各个概念当前的阶段（波谷/波中/波峰）
- **交互**：鼠标悬停显示该位置具体数值

```
┌──────────────────────────────────────────────────────────────┐
│  📈 核心概念对比走势（归一化，30天基准=100）                  │
│                                                                │
│  价格指数                                                     │
│   115 ┼──╱╲──────╱╲──────╱╲── AI算力 ── 波谷 ████████        │
│   110 ┼─╱──╲────╱──╲────╱──╲── 机器人  ── 波中 ██████         │
...
│    95 ┼───────────────────────────────────────                │
│       └──┬───┬───┬───┬───┬───┬───┬───┬───┬───┬──             │
│         D-9 D-7 D-5 D-3 D-1  D   (日期)                       │
│                                                                │
│  🟢 波谷: AI算力(vl=4)  🟡 波中: 机器人(vl=1)  🔴 下跌: HBM │
└──────────────────────────────────────────────────────────────┘
```

**作用：**
- 一眼看出哪些概念在波谷低位（可能机会），哪些在波峰高位（注意风险）
- 对比不同概念的强弱节奏差异——A 跌到底的时候 B 可能还在半山腰
- 发现板块轮动的节奏错位：比如 AI 算力到波谷时，机器人可能刚开始跌
