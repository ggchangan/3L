# Tushare 数据源迁移设计方案 v0.2 — 数据库+双账号策略
...
| 阶段 | 使用账号 | 工作内容 | 积分消耗估算 |
|------|---------|---------|------------|
| **一次性回填** | 15000积分账号 | 全量回填6年以上历史数据 | ~300万积分(一次性) |
| **每日增量** | 2000积分账号 | 每日追加最新数据+查询 | ~13万积分/天 |
...
**2000积分/天的预算分配：**
```
stock_daily:   50只 × 200积分 = 10,000
daily_basic:   50只 × 200积分 = 10,000
ths_daily:    ~500板块 × 免费 = 0
index_daily:   4指数 × 免费 = 0
ths_member:    ~50板块 × 免费 = 0
adj_factor:   50只 × 免费 = 0
trade_cal:     1次 × 免费 = 0
──────────────────────────────────
总计: ~20,000积分/天 (预算2000积分账号绰绰有余 ✅)
```

**注意:** 板块K线(ths_daily)、指数(index_daily)、成分股(ths_member)在Tushare中属于**免费接口**，不消耗积分。实际积分消耗主要是每日个股日线(200分/只)。
...
|------|------|------|------|
| **一次性回填(15000账号)** | | | |
| stock_daily (5000只, 6年) | 200/次 | 10次(500只/批) | 2,000 |
| daily_basic (5000只, 6年) | 200/次 | 10次 | 2,000 |
| ths_daily (~500板块, 6年) | 免费 | ~10次(50/批) | 0 |
| ths_member (~500板块) | 免费 | 1次 | 0 |
| ths_index | 免费 | 1次 | 0 |
| stock_basic | 免费 | 1次 | 0 |
| index_daily | 免费 | 4次 | 0 |
| adj_factor | 免费 | 10次 | 0 |
...
| **回填总计** | | | **~4,000积分** ✅ |
| **每日增量(2000账号)** | | | |
| stock_daily (50只自选) | 200/只 | 50只 | 10,000 |
| daily_basic (50只) | 200/只 | 50只 | 10,000 |
| ths_daily (全板块) | 免费 | 1次(批量) | 0 |
| 其他 | 免费 | — | 0 |
| **每日总计** | | | **~20,000积分** ✅ |

15000账号的积分只需一次回填，3只股票都花不了多少（因为每天只要1次调用能拿全部K线）。2000积分账号每天绰绰有余。
