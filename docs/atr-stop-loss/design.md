...
## v1.1 持仓止损维护规则（2026-08-01）

- **建仓初始止损**：使用买入日期之前可见的 K 线与买入价计算，避免未来数据参与建仓判断。
- **当前结构保护位**：持仓运行后，用最新结构计算保护位；涨幅只是触发复核的原因，不直接按涨幅百分比机械移动止损。
- **只上移、不下调**：已有手工止损是执行基准，系统建议不得把它调低。
- **建议与执行分离**：页面先展示初始止损、当前结构保护位和原因，只有用户点击“采用建议”才写入；后台不得静默覆盖。
- **触及止损先处置**：现价已经到达或跌破止损时，不再建议放宽止损，直接标记为风险处置场景。

这里的当前结构保护位沿用现有生产结构算法，仅作为人工决策建议。根据止损回测结论，在新的候选算法达到上线门槛前，不自动替换生产算法，也不批量更新持仓止损。
