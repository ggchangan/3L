...
2026-08-27 修正：P0.2 开始消费实验波段结构输出，至少区分两层上下文：

```text
structure / stage
  描述大级别结构和传统阶段，提供背景、风险偏好、压力/支撑推断。

wave_context.trading_wave / wave_context.trading_state
  描述当前交易波段，决定当下供需点是否允许按中继、反转、恐慌等语义解释。
```

这与 3L“按波段交易”的口径一致。典型约束：

```text
上涨趋势 + 当前下降交易波段：
  不能简单输出 bullish_continuation；
  只有重新出现缩量止跌、支撑承接或需求进入后，才可进入后续买点层判断。

下降趋势 + 当前上涨交易波段：
  不能简单输出 bearish_continuation；
  应先解释为下降趋势中的反弹波，并等待反弹衰竭或需求失败证据。
