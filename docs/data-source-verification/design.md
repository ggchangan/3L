...
> **口径约束（2026-07）：** provider 表示业务数据口径，transport 表示获取
> 通道。默认 provider 为 THS，优先级为 Tushare/THS DB → THS 实时通道 →
> 最近一次确认的 THS DB → THS 快照。东财是独立辅助 provider，只做对照
> 验证；仅当排行、K线和概念映射能力全部实现后才允许配置整源切换。禁止用
> 东财逐条补齐 THS，也禁止在一次业务计算中混合两个 provider。
>
> 当前收盘后“当日预估”仍是一个明确标记的历史复合路径（THS 历史基线 +
> 东财辅助快照），健康接口将其报告为 `legacy_pending_ths_migration`，不算作
> 权威主链数据。可通过 `SECTOR_AUXILIARY_PROVIDER=disabled` 禁用；下一阶段
> 用 THS 同日通道替换后移除此复合路径。
...
行业数据源从 push2test 切回同花顺 THS（`stock_board_industry_summary_ths`），概念保留 push2test。
...
|:-------|:----|:----|:----:|
| THS行业总数 | ≥80 | 90 | ✅ |
| 含电子化学品 | 存在 | `change_pct=-0.7%` | ✅ |
| 电子化学品涨跌幅 | 合理（>-20% <20%） | -0.7% | ✅ |
| 电子化学品上涨家数 | 存在 | 18 | ✅ |
| 电子化学品领涨股 | 非空 | 江化微 | ✅ |
| 半导体存在 | 存在 | `change_pct=-2.33%` | ✅ |
| 电机存在 | 存在 | `change_pct=3.12%` | ✅ |
| push2test概念命中 | ≥1 | 5/5 | ✅ |
...
| 检查项 | 预期 | 实际 | 结果 |
|:-------|:----|:----|:----:|
| sector_daily.json _push2test存在 | 非空 | dict | ✅ |
| _push2test行业数 | ≥80 | 90 | ✅ |
| 电子化学品chg | -0.7% | -0.7% | ✅ |
| last_updated | 非空 | 20260605 | ✅ |
| THS历史K线存在 | 非空 | dict | ✅ |
| get_sector_rankings() | 返回dict | 含电子化学品 | ✅ |

### L3 — 业务逻辑验证

| 检查项 | 预期 | 实际 | 结果 |
|:-------|:----|:----|:----:|
| get_mainline_data() lines非空 | >0 | 有数据 | ✅ |
| 电子化学品在lines中 | 存在 | 主线第1梯队 | ✅ |
| 电子化学品chg_1d | -0.7% | -0.7% | ✅ |
| 电子化学品chg_20d | >0 | 17.99% | ✅ |
| 电子化学品有stage | 非空 | 上涨 | ✅ |

### 结论

L1~L3 全部通过（23/23），L4（API /api/review/today）在非交易日返回0行，属于预期行为。数据源切换完成。
