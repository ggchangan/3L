...
脚本会自动完成：装 Docker → 配置报警 → 拉镜像 → 生成配置 → 启动服务。
...
> ⚠️ **部署完成 ≠ 可以直接使用。** 数据更新脚本只拉取**自选股+持仓股**的数据，
> 所以必须先添加股票，再拉数据。

**① 添加自选股**
访问 `http://<服务器IP>:8080/` → 登录 → 自选股管理 → 选方向、加股票

**② 添加持仓股**
`http://<服务器IP>:8080/holdings` → 录入持仓股票和成本/止损信息
...
```
自动完成：
- ✅ 行业映射全量生成（4680只申万二级行业）
- ✅ 搜索索引生成
- ✅ 自选股60天K线
- ✅ 中证全指200天 + 板块90天

> ⏱ 约 3-5 分钟。之后每个交易日 17:00 自动更新。
