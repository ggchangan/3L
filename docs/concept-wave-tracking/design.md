...
按**当前阶段**把概念分成五组，且每个阶段标签上标注回测胜率：

```
🟢 [ 波谷组(89.9%) ] → 缩量见底，买入信号
🟠 [ 波峰组(15.6%) ] → 高位滞涨，注意风险（A股见顶后常高位震荡）
🔵 [ 上涨组(61.7%) ] → 趋势延续，顺势持有
🔴 [ 下跌组(19.7%) ] → 筑底过程，往往迎来反弹
🟡 [ 波中组 ]         → 方向不明，继续观察
```
...
### 4.2 核心算法：概念板块V5波谷评分 → 5阶段判定

参照大盘 V5 评分法，适配到概念板块级别：

```
波谷评分 (vl_score 0~5):
  条件1: BIAS20 < -5%              → +1分 (价格远离EMA20)
  BIAS20 < -8%                      → 再+1分 (更极端)
  条件2: 量比 < 0.7                → +1分 (成交萎缩，供应枯竭)
  量比 < 0.5                        → 再+1分 (极度缩量)
  条件3: EMA10斜率从负转平(>-0.3)  → +1分 (下跌趋缓)
  EMA10斜率从负转正(>0)            → 再+1分 (开始拐头)
  条件4: 2σ偏差 < -1.5             → +1分 (统计极端)
  2σ偏差 < -2.0                    → 再+1分 (极度极端)

波峰评分 (pk_score 0~5):
  条件1: BIAS20 > 5%              → +1分 (远离均线向上)
  BIAS20 > 8%                      → 再+1分
  条件2: 放量滞涨(量比>1.5且涨幅<0.5%)  → +1分
  条件3: EMA10斜率从正转负        → +1分
  条件4: 2σ偏差 > 1.5             → +1分

5阶段判定：
  vl_score ≥ 5                    → 🟢 波谷 (极度超卖，100%胜率)
  vl_score ≥ 3 + 缩量(量比<0.7)   → 🟢 波谷 (89.9%胜率)
  pk_score ≥ 3 + 放量下跌          → 🟠 波峰 (15.6%胜率)
  EMA10斜率>0.3 + BIAS5>-3        → 🔵 上涨 (61.7%趋势延续)
  EMA10斜率<-0.3 + BIAS20<0       → 🔴 下跌 (19.7%继续跌，80%反弹)
  其他                             → 🟡 波中
