...
当前复盘的方向榜按“板块自身 10 日涨幅”排序，并把前 5、6–10 名作为两档候选。它可作为短期强度线索，但不符合 3L 知识库的 L1 动量主线定义，因此对外统一称为“10日板块强度候选”。

正式 L1 模型必须从全市场强势个股出发，再聚合到同花顺行业/概念；不能直接用板块自身涨幅替代。

## 2. 知识库口径与当前工程参数

> 知识库/早期实现多以 20 日作为动量统计窗口；实际复盘中发现 20 日对主线切换偏滞后。当前工程参数统一改为 10 个交易日，但 L1 的“从强势个股聚合到板块”的核心结构不变。

1. 计算全市场个股 10 日涨幅并取前列个股。基准为前 700 名，随市场容量扩张动态调整。
2. 剔除上市不足 10 个交易日的次新股。
...
| 输入 | 首选来源 | 最低要求 | 缺失处理 |
| --- | --- | --- | --- |
| 个股日线与复权价 | Tushare/THS 统一通道 | 至少 11 个有效交易日 | 个股不进入样本并计入缺失率 |
...
```text
eligible = 全市场股票
eligible = eligible.filter(上市天数 >= 10个交易日)
eligible = eligible.filter(10日行情完整)

top_n = max(700, round(len(eligible) * dynamic_top_ratio))
momentum_pool = eligible.sort_by(10日涨幅 desc).take(top_n)
...
代理榜继续返回 `model_type = sector_return_10d_proxy`、`is_l1_model = false`，二者不得共用“主线确认”的展示文案。
...
4. 与当前 10 日板块强度代理榜并行影子运行，记录差异及原因。
