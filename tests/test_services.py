...
class TestGetIndustryBoards:
    ...
    @patch('backend.services.market_service._cache_dir')
    @patch('backend.services.market_service.os.path.isfile')
    @patch('backend.services.market_service.os.path.getmtime')
    ...
    @patch('backend.services.market_service._cache_dir')
    @patch('backend.services.market_service.os.path.isfile')
    @patch('backend.services.market_service.json.load')
    @patch('backend.services.market_service.os.path.getmtime')
    ...
    @patch('backend.services.market_service._cache_dir')
    @patch('backend.services.market_service.os.path.isfile', return_value=False)
    @patch('backend.services.market_service._fetch_with_timeout')
    ...
    def test_handles_generic_exception_in_cache(self, mock_fetch,
        ...
        with patch('backend.services.market_service.os.path.isfile',
                   side_effect=[True, False]):
            with patch('backend.services.market_service.os.path.getmtime',
...
class TestGetBuySignals:
    ...
    @patch('backend.services.monitor_service.os.path.isfile', return_value=True)
    @patch('builtins.open', new_callable=mock_open,
    ...
    @patch('backend.services.monitor_service.os.path.isfile', return_value=False)
    @patch('backend.services.monitor_service.subprocess.run')
    @patch('backend.services.monitor_service.os.makedirs')
    @patch('backend.services.monitor_service.atomic_json_dump')
...
class TestGetTopSectors:
    ...
    @patch('backend.core.monitor_data.get_top_sectors_with_5d')
...
class TestSaveWatchlist(unittest.TestCase):
    ...
    @patch('backend.services.watchlist_service._save_watchlist_data')
    @patch('builtins.open', new_callable=mock_open)
    def test_saves_new_watchlist(self, mock_open_file, mock_save_data):
        """Happy path: saves watchlist data."""
        ...
        # 数据更新已交由 update_stock_data.py 统一处理
    ...
    def test_does_not_call_ensure_for_existing_stocks(
            self, mock_open_file, mock_save_data):
        """Only new (previously unseen) stocks handled by daily update."""
        from backend.services import watchlist_service
        fake_path = '/tmp/test_wl.json'
...
class TestSearchStocks:
    ...
    def _setup_mocks(self):
        ...
        self._patchers = [
            patch('backend.services.watchlist_service.ALL_STOCKS_PATH', '/fake/all_stocks.json'),
            patch('backend.services.watchlist_service.ALL_CODES_PATH', '/fake/all_codes.json'),
            patch('backend.services.watchlist_service.PINYIN_PATH', '/fake/pinyin.json'),
        ]
    ...
    @patch('backend.services.watchlist_service.os.path.isfile', return_value=True)
    @patch('builtins.open', new_callable=mock_open)
    ...
    @patch('backend.services.watchlist_service.os.path.isfile', return_value=False)
...
class TestGetHoldings:
    ...
    @patch('backend.services.holdings_service.os.path.isfile', return_value=True)
    @patch('builtins.open', new_callable=mock_open,
    ...
    @patch('backend.services.holdings_service.os.path.isfile', return_value=False)
...
class TestDownloadKbFile:
    ...
    @patch('backend.services.knowledge_service.os.path.isfile', return_value=False)
...
class TestReviewDataSourceConsistency:
    ...
    def test_scan_buy_signals_does_not_mutate_klines(self):
        ...
        try:
            ...
            with patch('backend.services.direction_service.get_active') as mock_active, \
