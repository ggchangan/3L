...
# ── 交易事件定义（买点idx → [(事件类型, idx, 价格, 标签)]） ──
# 找每个信号对应的卖出日
def find_exit_for_buy(buy_idx, kls, buy_signals_list, signal_idx):
    """找买点后的第一个卖点"""
    # 从回测输出手工标注，但也可以用逻辑找
    pass
...
# 硬编码交易事件（从回测结果精确提取）
trades = [
    {'buy_idx': 0, 'buy_date': '03-06', 'buy_price': 271.12, 'events': [
        ('sell', '03-12', 293.50, '阴包阳'),
    ]},
    {'buy_idx': 1, 'buy_date': '03-13', 'buy_price': 320.05, 'events': [
        ('sell', '03-19', 345.00, '阴包阳'),
    ]},
    {'buy_idx': 2, 'buy_date': '03-20', 'buy_price': 331.52, 'events': [
        ('stop', '03-24', 306.10, '止损'),
        ('buyback', '03-25', 375.17, '买回'),
        ('sell', '03-30', 400.30, '加速'),
    ]},
    {'buy_idx': 3, 'buy_date': '03-25', 'buy_price': 375.17, 'events': [
        ('stop', '03-26', 361.02, '止损'),
        ('buyback', '03-27', 381.89, '买回'),
        ('sell', '04-01', 371.98, '大阴线'),
    ]},
    {'buy_idx': 4, 'buy_date': '03-27', 'buy_price': 381.89, 'events': [
        ('sell', '04-01', 371.98, '大阴线'),
    ]},
    {'buy_idx': 5, 'buy_date': '04-10', 'buy_price': 447.24, 'events': [
        ('sell', '04-15', 501.20, '动力弱'),
    ]},
    {'buy_idx': 6, 'buy_date': '04-15', 'buy_price': 501.20, 'events': [
        ('sell', '04-20', 515.00, '动力弱'),
    ]},
    {'buy_idx': 7, 'buy_date': '04-20', 'buy_price': 515.00, 'events': [
        ('sell', '04-23', 494.69, '阴包阳'),
    ]},
    {'buy_idx': 8, 'buy_date': '04-21', 'buy_price': 514.60, 'events': [
        ('sell', '04-23', 494.69, '阴包阳'),
    ]},
    {'buy_idx': 9, 'buy_date': '05-06', 'buy_price': 577.29, 'events': [
        ('sell', '05-08', 597.71, '阴包阳'),
    ]},
    {'buy_idx': 10, 'buy_date': '05-07', 'buy_price': 616.51, 'events': [
        ('sell', '05-08', 597.71, '阴包阳'),
    ]},
    {'buy_idx': 11, 'buy_date': '05-14', 'buy_price': 721.65, 'events': [
        ('sell', '05-19', 699.16, '阴包阳'),
    ]},
    {'buy_idx': 12, 'buy_date': '05-20', 'buy_price': 695.96, 'events': [
        ('stop', '05-21', 656.10, '止损'),
    ]},
]
...
sv.append(f'<text x="{W/2}" y="20" text-anchor="middle" font-family="sans-serif" font-size="17" fill="#ffffff" font-weight="bold">德明利(001309) 3L优化回测 — 13笔交易全景</text>')
sv.append(f'<text x="{W/2}" y="33" text-anchor="middle" font-family="sans-serif" font-size="10" fill="#888888">🔴红=买入  🟢绿=卖出  🔵蓝=止损  🟡橙=买回</text>')
...
# 先收集所有事件并排序，防止遮挡
all_marks = []  # (idx, color, label, y_price, type_order)

for t_idx, trade in enumerate(trades):
    num = t_idx + 1
    buy_idx = trade['buy_idx']
    bi = buy_signals[buy_idx]['idx']
    bp = trade['buy_price']
    bd = trade['buy_date']
    ...
    for evt in trade['events']:
        etype, edate_str, eprice, elabel = evt
...
for mark in all_marks:
    ...
    clr = mark[1]; label = mark[2]; num = mark[5]
...
OUT = '/home/ubuntu/www/review_charts/德明利_交易记录.svg'
